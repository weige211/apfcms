...
public class UserController {
	...
	public String addUserInfo(UserDto u, HttpSession session, Model model) {
		try {
			...
			user.setCreateuser(oper.getUsername());// ���õ�ǰ��¼��Ϊ������
			userService.add(user);
			List<Sys_User> list = userService.getAll();
			model.addAttribute("list", list);
			return "user/users";
		} catch (Exception e) {
			logger.error("添加用户信息失败！" + e.getMessage());
			model.addAttribute("erro", "添加用户信息失败！");
			return "erro";
		}
	...
	public String updateUserInfo(UserDto u, HttpSession session, Model model) {
		try {
			...
			Sys_User oper=(Sys_User)session.getAttribute("user");
			user.setUpdateuser(oper.getUsername());//记录用户名

			userService.modifyByKey(user);
		} catch (Exception e) {
			logger.error("修改用户名失败！" + e.getMessage());
			model.addAttribute("erro", "修改用户名失败!"+e.getMessage());
			return "erro";
		}
		try {
			List<Sys_User> list = userService.getAll();
			...
			model.addAttribute("erro", "获取用户信息失败！"+e.getMessage());
			return "erro";
		}
